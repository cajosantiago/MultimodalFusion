...
def plot_training_stats(train_results, val_results):
    ...
    plt.legend()
    save_plot(fig_acc, f'accuracy_plot_{timestamp}.png')
    plt.show()
    ...
    plt.legend()
    save_plot(fig_metrics, f'metrics_plot_{timestamp}.png')
    plt.show()
...
def late_predictions(epoch, fusion_data ):
    predictions_dir = "plots/experiment_02"
    
    os.makedirs(predictions_dir, exist_ok=True)
    filename = 'predictions.txt'
    
    predictions_file = os.path.join(predictions_dir, filename)

    # Determine whether it's the first epoch
    first_epoch = epoch == 1

    # Clear the file only if it's the first epoch
    if first_epoch:
    ...
    with open(predictions_file, 'a') as file:    
        for i in range(len(fusion_data["Final"]["combination"])):
            ...
            patient_id = fusion_data["Final"]["patient_id"][i]
            
            combination_str = str(combination)
            
            if combination_str not in combinations:
                combinations[combination_str] = {"patient_id": [], "label": [], "prediction": []}
            
            combinations[combination_str]["patient_id"].append(patient_id)
            combinations[combination_str]["label"].append(label)
            combinations[combination_str]["prediction"].append(prediction)
        
        for combination_str, data in combinations.items():
            patient_ids = data["patient_id"]
            labels = data["label"]
            predictions = data["prediction"]   
            file.write(f'Combination {combination_str} - {len(data["patient_id"])} patients \n')

            
            for patient_id, prediction, label in zip(patient_ids, predictions, labels):
                file.write(f'Patient ID: {patient_id}, Prediction: {prediction}, True Label: {label}\n')

    ### BELOW IS FOR EARLY FUSIIIIIIIIIIIIIIIIIIIION
       
def write_predictions_to_file(epoch, combination_patients):
    predictions_dir = "plots"
    ...
    with open(predictions_file, 'a') as file:
        ...
        for combination, patients in combination_patients.items():
            # Calculate the number of patients for the current combination
            num_patients = len(patients)

            # Write the combination information with the patient count
            file.write(f'Combination {combination} - {num_patients} Patients:\n')

            # Write patient information
            for patient_id, pred, true in patients:
                file.write(f'Patient ID: {patient_id}, Prediction: {pred}, True Label: {true}\n')
