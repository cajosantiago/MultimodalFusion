...
def get_args_parser():
    ...
    parser.add_argument('--opt', default='adam', type=str, metavar='OPTIMIZER', choices=['adamw', 'sgd', 'adam'],
...
def main(args):
    ...
    noise = 0
    if noise == 1:
        train_csv = 'Files/train_noise_MR.csv'
    else:
    ...
    if(warmup):
        ...
        dataset_MR_warmup = data_load.FeatureDataset(balanced_dataset,'Files/MR_scans_dataset.csv', bag_size=47)
        data_loader_MR_warmup = DataLoader(dataset_MR_warmup, batch_size=1, 
        ...
        if(progressive_warmup):
            ...
            dataset_MR_warmup1 = data_load.FeatureDataset(warmup_train1,'Files/MR_scans_dataset.csv', bag_size=47)
            data_loader_MR_warmup1 = DataLoader(dataset_MR_warmup1, batch_size=1, 
                                     shuffle=True, num_workers=args.num_workers, 
                                     pin_memory=args.pin_mem, drop_last=False)
            #create dataloader for dataset2
            dataset_MR_warmup2 = data_load.FeatureDataset(warmup_train2,'Files/MR_scans_dataset.csv', bag_size=47)
            data_loader_MR_warmup2 = DataLoader(dataset_MR_warmup2, batch_size=1, 
                                     shuffle=True, num_workers=args.num_workers, 
                                     pin_memory=args.pin_mem, drop_last=False)
    ...
    dataset_MR_train = data_load.FeatureDataset(train_csv,'Files/MR_scans_dataset.csv', bag_size=47)
    data_loader_MR_train = DataLoader(dataset_MR_train, batch_size=1, 
                             shuffle=True, num_workers=args.num_workers, 
                             pin_memory=args.pin_mem, drop_last=False)
    
    dataset_MR_val = data_load.FeatureDataset('Files/val_dataset.csv','Files/MR_scans_dataset.csv', bag_size=47)
    data_loader_MR_val = DataLoader(dataset_MR_val, batch_size=1, 
    ...
    step_lr_scheduler = StepLR(optimizer, step_size=30, gamma=0.01)
    ...
    for epoch in range(args.start_epoch, (args.epochs + args.cooldown_epochs)):
        # Train the model
        if warmup and epoch < 30:
            if progressive_warmup and epoch < 10:
                dataset = data_loader_MR_warmup
                criterion = criterion_warmup
            elif progressive_warmup and epoch < 20 and epoch >= 10:
                dataset = data_loader_MR_warmup1
                criterion = criterion_warmup1
            elif progressive_warmup and epoch < 30 and epoch >= 20:
                dataset = data_loader_MR_warmup2
                criterion = criterion_warmup2
        else:
            warmup = False
            dataset = data_loader_MR_train
